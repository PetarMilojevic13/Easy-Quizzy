...
def choice_category_question_number(request):
    ...
    Ova funkcija vadi izabrani broj pitanja,kategoriju,pocetno pitanje, zamensko pitanje, kao i sva ostala pitanja u zavisnosti od nivoa korisnika, izabrane kategorije i broja pitanja!
        :param request:
        :return:
    """
    #Test
    ...
    level_current_user = myLevel(request.user.username)

    #print(level_current_user)
...
def test_end(request):
    ...
    if 'test ponovo' in request.POST:
        ...
        role_user = myRole(request.user.username)
        ...
        if(role_user==-1):
            return render(request, 'EasyQuizzy/picking_category_number_of_questions_singleplayer.html',
                          {'data':data,'range':range(length_range)})
...
def myLevel(korIme):
    ...
    """
    Pomoćna funkcija koja vraća nivo korisnika na osnovu njegovog korisničkog imena
    Samo registrovani korisnici imaju nivo, gosti nemaju
    :param korIme:
    :return:
    """
    if korIme == '':
