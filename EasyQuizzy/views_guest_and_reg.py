...
def dayQuestion(request):
    print('u views_guest_and_reg.dayQuestion')
    return
    ...
    if request.method == 'POST':
        ...
        for user in adminUsers:
            if user.idkor.korisnicko_ime == korIme and user.idkor.lozinka == password and user.idkor.vazeci == 1:
                ...
                print('pre create user')
                userRet = User.objects.create_user(username=korIme, password=password)
                print('posle create user')
                login(request, userRet)
                print('nasao admina')
                return redirect('main')
...
def questionSuggestion(request):
    ...
    if request.method == 'POST':
        ...
        if text == '':
            context['text'] = "Uneli ste prazno pitanje!"

        elif checkIfQuestionExists(text) == True:
            ...
            context['text'] = "Uspešno ste predložili pitanje!"
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if(prezime==""):
            context['message'] = 'Prezime nije popunjeno!'
            return HttpResponse(template.render(context, request))
